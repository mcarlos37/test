...
@Entity
@NamedQueries({
    @NamedQuery(name = "SmsMessage.findAllBySmsMessageType", query = "SELECT s FROM SmsMessage s WHERE s.smsMessageProcessedStatus = :smsMessageProcessedStatus AND s.smsMessageType = :smsMessageType AND s.processedAt BETWEEN :initDate AND :endDate"),
