...
@Entity
@NamedQueries({
    @NamedQuery(name = "SmsMessage.findAllBySmsMessageType", query = "SELECT s FROM SmsMessage s WHERE s.smsMessageType = :smsMessageType AND s.processedAt > :initDate AND s.processedAt < :endDate"),
